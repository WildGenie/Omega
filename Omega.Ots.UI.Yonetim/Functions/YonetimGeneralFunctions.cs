...
using Omega.Ots.UI.Yonetim.Properties;
...
namespace Omega.Ots.UI.Yonetim.Functions
{
    public static class YonetimGeneralFunctions
    {
        ...
        public static bool CreateDatabase<TContext>(string splashCaption, string splashDescription, string onayMesaj, string bilgiMesaj) where TContext : DbContext, new()
        {
            using (var con = new TContext())
            {
                ...
                catch (SqlException ex)
                {
                    ...
                    {
                        case 5170:
                            Messages.HataMesaji("Veritabanı Dosyalarının Yükleneceği Klasorde Aynı İsimde Bir Dosya Zaten Var. Lütfen Kontrol Ediniz.\n\n" + ex.Message);
                            break;
                        default:
